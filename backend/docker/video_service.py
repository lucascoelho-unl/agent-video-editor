...
from .config import CONTAINER_VIDEOS_PATH, CONTAINER_RESULTS_PATH, ALLOWED_VIDEO_EXTENSIONS
    ...
    def upload_video(self, file: UploadFile) -> dict:
        ...
        try:
            ...
            success = self.docker.copy_file_to_container(
                temp_file_path, 
                f"{CONTAINER_VIDEOS_PATH}/{file.filename}"
            )
            
            if not success:
                raise HTTPException(
                    status_code=500,
                    detail="Failed to upload file to container"
                )
